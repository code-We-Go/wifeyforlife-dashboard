...
const RoyaltyPointsTable: React.FC = () => {
  ...
  const [editing, setEditing] = useState<RoyaltyBonus | null>(null);
  ...
  const handleDelete = async (id: string) => {
    ...
    const res = await fetch(`/api/loyalty/loyaltyPoints/${id}`, { method: 'DELETE' });
    if (res.ok) setRoyaltyPoints(royaltyPoints.filter(r => r._id !== id));
    else alert('Failed to delete');
  };

  const handleRoyaltyPointUpdated = (updated: RoyaltyBonus) => {
    setRoyaltyPoints(royaltyPoints.map(r => (r._id === updated._id ? updated : r)));
    setEditing(null);
  };
  ...
  return (
    <>
          ...
          {royaltyPoints.map(rp => (
            <tr key={rp._id} className='text-center'>
              ...
              <td className="border px-4 py-2">
                <button onClick={() => setEditing(rp)} className="text-blue-600 mr-2">Edit</button>
                <button onClick={() => handleDelete(rp._id!)} className="text-red-500 ml-2">Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
      ...
      {editing && (
        <EditRoyaltyPointModal royaltyPoint={editing} onClose={() => setEditing(null)} onRoyaltyPointUpdated={handleRoyaltyPointUpdated} />
      )}
    </>
  );
};
